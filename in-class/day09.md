...
There are several reasons why we might use a SLAM algorithm over simple filtering, including long-term deployment duration and complex task definitions that benefit from a richer state representation. 
...
* **Cycles**: The robot might traverse a map in an arbitrarily challenging way, potentially never back-tracking or revisiting parts of the map from unexpected directions that can lead to large cumulative errors. 
...
Estimating the entire map, $$m = {\mathbf{m}_i}$$, is intractable (consider a simple 100 x 100 grid; that's 10,000 cells, which can take on one of two values, leading to $$2^{10,000}$$ different map possibilities). So instead, we choose to decompose this problem into a set of estimation problems:
...
where the boxed elements must be inferred, and the unboxed elements are observables.
...
In SLAM, we must also consider this same challenge with respect to our environment. Indeed, a SLAM algorithm needs to be able to reason about whether some observed object is related to previously observed objects. This is known as the _correspondence problem_, and we might consider explicitly correspondence variables in our estimation problem:
...
We will additionally make a simplifying assumption for our derivation: the correspondences between landmarks will be known. 
...
which express the original belief that the robot is initialized at the origin of the world and none of the landmark locations are known.
...
The special case here is that in the prediction step, we want to preserve our uncertainty (covariance) over our landmarks (and not reduce this uncertainty to 0). Thus, we compose our Jacobian out of a block of partial derivatives for robot pose, and an identity matrix for all elements of landmarks. 
...
such that $$F$$ is composed of 3 x 3 identity matrix (over robot state) and $$3N$$ columns of 0s, where $$N$$ is the number of landmarks.

We can now use $$F$$ in our estiamte of our Jacobian for this set-up:
...
During the update step, we want to update our estimate of where the each landmark is (and associated covariance) as well as where our robot is (and associated covariance). To do this, we can sequentially solve for landmark estimates, and then for the robot, following this set of steps:

1. Given the observation, estimate the location of each landmark in visual range.
2. Compute the residual between the estimated location of each landmark from the EKF and from the single drawn observation.
3. Construct a measurement matrix (jacobian) which transforms the residual observation vector back into the full state space.
...
This allows us to compute a residual and innovation (and thus Kalman gain) for every landmark observation. 
...
$$
\mathcal{X}_t = \hat{\mathcal{X}_t} + K_t^i(z_t^i - h(\hat{\mathcal{X}}_t, i))
$$
...
In practice, we might want to choose the "best" landmark observation if several take place at a single timestep. We can choose the best landmark by picking the smallest observation residual with the smallest associated covariance, for instance. This saves us a little time by only needing to compute single Kalman gain and posterior for each timestep.
