...
* **Part 1: Sense and Act** Given the set-up for our system, state/compute the following (note: you might find this easiest to express in tables).
  ...
  * The probability of each label given each category (e.g., probability that an object is labelled metal when it is metal, labelled metal when it is plastic, etc.), also known as the classification accuracy, e.g., $$\mathcal{P}(Label\vert Category)$$

  * The probabiilty of a particular type of salvage given a label e.g., $$\mathcal{P}(Category \vert Label)$$ (Note: this is a tough one! You might want to visit the Robotics Book for more details, or work with friends to justify your approach)
