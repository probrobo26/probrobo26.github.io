...
$$\mathcal{P}(A|B) = \frac{\mathcal{P}(A,B)}{\mathcal{P}(B)} = \frac{\mathcal{P}(B|A)\mathcal{P}(A)}{\mathcal{P}(B)}$$
...
$$\mathcal{P}(B) = \sum_i\mathcal{P}(B|A_i)\mathcal{P}(A_i)$$
...
Ok, let's try this out on a robotics problem with sensing and action commands. For those who would like, there is a [slide deck version of this discussion that can be viewed here](). To do this, there is one extension of Bayes Rule I want to present (which will allow us to incorporate both our Sense and Action spaces):

$$\mathcal{P}(A|B,C) = \frac{\mathcal{P}(B|A,C)\mathcal{P}(A|C)}{\mathcal{P}(B|C)}$$

**Philosophy**
[Image of the world-robot models; sense/action framework]
...
$$\mathcal{P}(z_t = 1 | x_t = 1) = 3/5 $$
$$\mathcal{P}(z_t = 1 | x_t = 0) = 1/5 $$
...
$$\mathcal{P}(x_t = 1 | x_{t-1} = 0, u_t = 0) = 0 $$
$$\mathcal{P}(x_t = 1 | x_{t-1} = 1, u_t = 0) = 1 $$
$$\mathcal{P}(x_t = 1 | x_{t-1} = 0, u_t = 1) = 4/5 $$
$$\mathcal{P}(x_t = 1 | x_{t-1} = 1, u_t = 1) = 1 $$
...
$$\mathcal{P}(x_1 = 1 | z_1 = 1, u_1 = 0) = \text{ ?? }$$
...
$$\mathcal{P}(x_1 = 1 | z_1 = 1, u_1 = 0) = \frac{\mathcal{P}(z_1 = 1 | x_1 = 1, u_1 = 0)\mathcal{P}(x_1 = 1|u_1 = 0)}{\mathcal{P}(z_1 = 1| u_1 = 0)}$$

Now, let's take this one piece at a time, and substitute in what we know:

$$\mathcal{P}(z_1 = 1 | x_1 = 1, u_1 = 0) = \mathcal{P}(z_1 = 1 | x_1 = 1) = 3/5$$

Our _observation model_ or _likelihood_ portion of our system is pretty straightforward -- we can nearly read it off our starting conditions! But there is one key thing we did here to help us: we realized that our observation was independent of our action; our action has no bearing on what is observed, allowing us to completely ignore that term in our conditional probabilty statement.

$$\mathcal{P}(x_1 = 1|u_1 = 0) = \mathcal{P}(x_1 = 1, x_0 = 1|u_1 = 0) + \mathcal{P}(x_1 = 1, x_0 = 1|u_1 = 0)$$
$$ = \mathcal{P}(x_0 = 0 | u_1 = 0)\mathcal{P}(x_1 = 1| x_0 = 0, u_1 = 0) + \mathcal{P}(x_0 = 1 | u_1 = 0)\mathcal{P}(x_1 = 1| x_0 = 1, u_1 = 0)$$
$$ = 1/2 \times 0 + 1/2 \times 1 $$
$$ = 1/2 $$

Our _action model_ is a little trickier, because the probability of our current state given our action is contingent on what our prior state was. So we expand our probability statement to include what our prior state could have been. From there, we just re-arrange the terms until we get something that we can plug in directly: our prior probability on state and our action model.

And so, our final probability (with some normalizing factor $$\mu$$) is simply:

$$\mathcal{P}(x_1 = 1 | z_1 = 1, u_1 = 0) = \mu \times 3/10$$
or 
$$\mathcal{P}(x_1 = 1 | z_1 = 1, u_1 = 0) \propto  3/10$$
...
Finish the door-opening robot problem from today by computing the _normalized_ probabilities of different events. You might find the additional notes in the slides helpful for this! (hint: the answers are given there, to help you calibrate your approach)
