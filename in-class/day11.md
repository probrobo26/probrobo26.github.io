---
title: "Day 11: SLAM III: EKF-SLAM Wrap Up and Extensions"
toc_sticky: true
toc_data:
  - title: Today
  ...
  - title: EKF-SLAM Details (Continued)
    link: in-class/day11/#ekf-slam-continued
  - title: Beyond EKF-SLAM
    link: in-class/day11/#beyond-ekf-slam
  - title: Deep Dive Project Worktime
    link: in-class/day11/#deep-dive-project-worktime
---
...
## For Next Time
* Keep working on your [Deep Dive Projects](../projects/deepdive_1.md). Brief check-ins will be held next class! 
...
## Beyond EKF-SLAM
EKF-SLAM is a powerfully simple idea: add states to track in a Kalman filter that describe the environment. But what happens when the state space of the world is...more complicated? 

There are some key limitations of EKF-SLAM:
* Polynomial computations in the size of the state and observation spaces (for large state vectors, this algorithm is computationally slow)
* It operates best when linearization is a good approximation of an underlying system; otherwise it can diverge
* It requires a relatively sparse observation space of high-quality
* It is sensitive to loop closures (revisiting a known landmark after a long time)

In a world where we have a different sensing regime, want to keep track of more environmental information, or want stronger convergence guarantees, we might select a different SLAM algorithm to use.

Other common families of SLAM algorithms include:
* [FastSLAM](https://ai.stanford.edu/~koller/Papers/Montemerlo+al:AAAI02.pdf) -- A direct extension to EKF-SLAM that extends the number of landmarks that the filter can track; reduces computational complexity significantly by leveraging particle filtering techniques
* [GraphPose SLAM](https://ieeexplore.ieee.org/stamp/stamp.jsp?arnumber=5681215&casa_token=MfDH4CGr2HEAAAAA:xDTxOeTujyv8MrXdU7UHRkOz6NtM0iCZ-8SGEvJ_krbfIOOSNMla3d_BDVk9t4VcL_wvHCn5r8Q&tag=1) -- in which factor graphs are used to represent an environment, and graph-based optimization techniques can be utilized for computational speed-up on very large maps; particularly good at loop closures
* Visual SLAM (e.g., Monocular SLAM, [ORB-SLAM](https://ieeexplore.ieee.org/stamp/stamp.jsp?arnumber=7219438&casa_token=W5IQVIAmwDUAAAAA:Qd61H1XZkg1tM66sq0XnKZVrLn68HWW-zDt-7uVX45MXYbxNT6XsGg1ObjN8tyg8JSkuwbLIKDg)) -- uses visual feature spaces within a SLAM framework for dense, rich reconstructions of an environment; can be done though filtering techniques or graph-based techniques
...
The remainder of class is designed to be time to work on your deep dive projects.